...
* ### 非功能性
    ...
    易用性：指用户使用软件的容易程度。(最好是使用按键对应相应的功能)，测试最好是从用户角度出发，进行用户测评。
...
* #### 三种传参方式
    ...
    * #### 按值传递
    > 调用函数本身不对实参进行操作。即使形参的值在函数中发生了变化，实参的值也不会受到影响，仍为调用之前的值。相当于把实参赋值一份给形参，然后调用后内存回收。
    ...
    * #### 按地址传递
    > 把实参的存储地址传递给对应的形参，从而得到形参指针和实参指针指向同一个地址。被调用函数中对形参指针所指向的地址中内容的任何改变都会影响实参。
    ...
    * #### 引用传递
    > 以引用为参数，既可以使得对形参的任何操作都改变相应的实参，又使得函数调用显得方便。

    > 引用传递方式是在函数定义时在形参前面加上引用运算符“&”。引用相当于实参的别名，引用的改变就是实参的改变。
...
# 第03章 课程总结
    ...
    本课程从编程质量属性的说起，阐述了高质量编程的重要性。之后对编写代码过程中的编码风格做了一些规范，从一些规则中开始逐渐的形成自己的编码风格。进而提高编码质量。
